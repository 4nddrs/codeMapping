...
| Cambrian-S | Stage-3 video instruction tuning: the same launcher with the `cambrians_7b_s3.sh` flags, 2 optimizer steps over an 8-row video fixture (6 mp4 + 2 frame directories, 8 frames each) — MIV token layout, full-model AdamW, `model.pth.zstd` export; 2 further capture shims (TPU-only bilinear resize, per-tensor state-dict sync) | exit 0 · 2/2 steps · logged loss 2.098 · model.pth.zstd 6.6 GB | [S3 video](projects/cambrian_s/s3_video.html) |
| MolmoAct2 | 3 bounded command flows (DROID and BimanualYAM inference servers, ManiSkill zero-shot evaluation) in the repository group; deployment-path only, no robot hardware on this host | 3/3 exit 0 | [Grouped flows](index.html#molmoact2) |
